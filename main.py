...
def generar_contraseña(longitud, mayus, nums, esp):
    ...
    # Condicionales para añadir preferencias del usuario
    if mayus:
    ...
    # Bucle seguro para estructurar la contraseña aleatoria
    contrasena_final = "".join(secrets.choice(caracteres) for _ in range(longitud))
    return contrasena_final

def main():
    while True: # Estructura repetitiva para mantener el menú activo
        mostrar_menu()
        ...
        if opcion == "1":
            try:
                ...
                if longitud < 8:
                    print("⚠️ La longitud mínima debe ser de 8 caracteres.")
                    continue
